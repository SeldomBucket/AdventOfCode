...
def day_5_part_1():
	print("\nDAY 5 PART 1")
	filename = "AdventOfCodeDay4Input"
	lines = read_input_file("AdventOfCodeDay5Input")
	coords_covered_by_2_or_more_lines = set()
	coords_covered_by_1_line = set()
	for line in lines:
		coordinates = line.split(" -> ")
		x1 = int(coordinates[0].strip().split(',')[0])
...
def day_5_part_2():
	...
	for line in lines:
		...
		if (y1 == y2):
			...
			if (x2 > x1):
				x_coords_diagonal_list = list(range(x1, x2+1))
			else:
				x_coords_diagonal_list = list(range(x1, x2-1, -1))
